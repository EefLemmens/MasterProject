...
inputfile = 'Hospital_Billing.csv'
inputfile2 = 'Only Outliers Duration Start Hospital Billing.csv'
outlierfile_start = 'Outliers file Duration Start Hospital Billing.csv'
nonoutlierfile_start = 'NonOutliers file Duration Start Hospital Billing.csv'
...
firsttimestamps = []
caseIDsfirst = []
first = completetimestamp2[0]
firstact = 0
savedcaseid = caseID[0]
for i in range(0,len(caseID)):
    if caseID[i] != caseID[i - 1]:
        savedcaseid = caseID[i]
        for l in range(firstact, i):
            if completetimestamp2[l] < first:
                first = completetimestamp2[l]
        temporarily = [caseID[i-1], first]
        firsttimestamps.append(temporarily)
        first = completetimestamp2[i]
        firstact = i

dateswithoutliers = []
for i in range(0, len(outlierdata)):
    if outlierdata[i][1] == 1:
        dateswithoutliers.append(outlierdata[i][0])

for i in range(0,len(firsttimestamps)):
    if firsttimestamps[i][1] in dateswithoutliers:
        if firsttimestamps[i][0] not in outliercases_start:
            outliercases_start.append(firsttimestamps[i][0])

#Find case ID's that are not outliers
for i in range(0,len(caseID)):
    if caseID[i] not in outliercases_start:
        nonoutliercases_start.append(caseID[i])

#Make CSV's outliers and non outliers

with open(outlierfile_start,'w', newline ='') as f:
    writer = csv.writer(f)
    temp = []
    ...
    for i in range(0, len(caseID)):
        if caseID[i] in outliercases_start:
            temp = []
...
with open(nonoutlierfile_start, 'w', newline='') as f:
    writer = csv.writer(f)
    temp = []
    for i in range(0,len(firstline)):
        temp.append(firstline[i])
